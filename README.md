...
```bash
pipx install target-shopify-v2
```
...
You can easily run `target-shopify-v2` by itself or in a pipeline using [Meltano](https://meltano.com/).
...
tap-carbon-intensity | target-shopify-v2 --config /path/to/target-shopify-v2-config.json
```
...
Create tests within the `target_shopify_v2/tests` subfolder and
  then run:
...
You can also test the `target-shopify-v2` CLI interface directly using `poetry run`:

```bash
poetry run target-shopify-v2 --help
```
...
```bash
# Test invocation:
meltano invoke target-shopify-v2 --version
# OR run a test `elt` pipeline with the Carbon Intensity sample tap:
meltano elt tap-carbon-intensity target-shopify-v2
```
